...
static int mtk_drm_esd_recover(struct drm_crtc *crtc)
{
	...
	last_backlight = panel_ext->params->last_backlight;
	...
	msleep(100);
	mtk_crtc_pkt_create(&cmdq_handle, crtc, mtk_crtc->gce_obj.client[CLIENT_CFG]);
	mtk_ddp_comp_io_cmd(output_comp, cmdq_handle, DSI_SET_BL, &last_backlight);
	pr_err("esd set backlight: %d\n", last_backlight);
	cmdq_pkt_flush(cmdq_handle);
	cmdq_pkt_destroy(cmdq_handle);
...
int mtk_drm_esd_testing_process(struct mtk_drm_esd_ctx *esd_ctx, bool need_lock)
{
		...
		do {
			...
			if (strstr(wt_saved_command_line,"n28_gc7272_dsi_vdo_hdp_txd_sharp")){
				DDPPR_ERR("[ESD] check TP gcore_tp_esd\n");
