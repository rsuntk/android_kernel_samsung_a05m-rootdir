...
static inline bool chip_is_husb311(struct rt1711_chip *chip)
{
	return chip->chip_pid == HUSB311_PID && chip->chip_vid == HUSB311_VID;
}
...
static void rt1711_irq_work_handler(struct kthread_work *work)
{
	...
	do {	//N28 bring up
		regval = tcpci_alert(chip->tcpc,false);
		if (regval)
			break;
...
static int rt1711_get_power_status(struct tcpc_device *tcpc)
{
	...
	if (chip_is_cps8851(chip))
		return cps8851_get_power_status(tcpc);
...
#ifndef RT_KERNEL_VER_OVER_6_6_0
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (ret < 0)
		pr_err("%s no intr-gpio info\n", __func__);
	else
		chip->irq_gpio = ret;
#else
	ret = of_property_read_u32(np,
		"rt1711pd,intr-gpio-num", &chip->irq_gpio);
	if (ret < 0)
...
static int rt1711_tcpcdev_init(struct rt1711_chip *chip, struct device *dev)
{
	...
	if ((chip->chip_id > RT1711H_DID_B) || chip_is_husb311(chip))
	...
	if ((chip->chip_id >= RT1715_DID_D) || chip_is_husb311(chip))
...
static int rt1711_i2c_probe(struct i2c_client *client)
{
	...
	if (chip_is_husb311(chip)) {
		data = 0x00;
		rt1711_write_device(client, RT1711H_REG_BMC_CTRL, 1, &data);
		pr_err("%s set low pwr mode\n", __func__);
	}
#endif	/* CONFIG_TCPC_LOW_POWER_MODE */

#ifdef CONFIG_N28_CHARGER_PRIVATE
	if (chip_is_husb311(chip)) {
		hardwareinfo_set_prop(HARDWARE_PD_CHARGER, "HUSB311ACC");
	} else if (chip_is_cps8851(chip)) {
		hardwareinfo_set_prop(HARDWARE_PD_CHARGER, "CPS8851");
	}
#endif
	pr_info("%s probe OK!\n", __func__);
	return 0;
	...
	if (chip != NULL) {
		...
		i2c_smbus_write_byte_data(
			client, RT1711H_REG_SWRESET, 0x01);
                if (chip_is_husb311(chip)) {
                        i2c_smbus_write_byte_data(client, RT1711H_REG_BMC_CTRL, 0x00);
                }
	}
}
