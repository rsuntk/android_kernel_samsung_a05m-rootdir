...
#if defined (CONFIG_N28_CHARGER_PRIVATE)
int tcpci_typec_cc_orientation = 0;
EXPORT_SYMBOL(tcpci_typec_cc_orientation);
#endif
				...
				unsigned long event, void *data)
{
...
#if defined (CONFIG_N28_CHARGER_PRIVATE)
	if (noti && rpmd) {
		rpmd->pd_conntected = noti->pd_state.connected;
	}
#endif
	mt_dbg(rpmd->dev, "event = %lu, idx = %d\n", event, idx);
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		if (old_state == TYPEC_UNATTACHED &&
...
#if defined (CONFIG_N28_CHARGER_PRIVATE)
			dev_info(rpmd->dev, "%s Charger plug out vbus level=%d\n", __func__, rpmd->tcpc[idx]->vbus_level);
			if(rpmd->tcpc[idx]->vbus_level == TCPC_VBUS_VALID)
				rpmd->tcpc[idx]->vbus_level = TCPC_VBUS_INVALID;
#else
			...
			typec_set_orientation(rpmd->typec_port[idx],
			    ...
			    new_state == TYPEC_UNATTACHED) {
#if defined (CONFIG_N28_CHARGER_PRIVATE)
			dev_info(rpmd->dev, "%s OTG plug out vbus level=%d\n", __func__, rpmd->tcpc[idx]->vbus_level);
			if(rpmd->tcpc[idx]->vbus_level == TCPC_VBUS_VALID)
				rpmd->tcpc[idx]->vbus_level = TCPC_VBUS_INVALID;
#else
		...
		if (new_state == TYPEC_UNATTACHED){
			tcpci_typec_cc_orientation = 0;
			dev_notice(rpmd->dev,"%s typec_unattached\n",__func__);
		} else {
			tcpci_typec_cc_orientation = noti->typec_state.polarity + 1;
			dev_notice(rpmd->dev,"%s typec_attached(%d)\n",__func__,tcpci_typec_cc_orientation);
		}
#endif
		if (new_state == TYPEC_UNATTACHED) {
	...
	case TCP_NOTIFY_PR_SWAP:
		...
		if (chg_psy == NULL || IS_ERR(chg_psy)) {
			dev_info(rpmd->dev,"%s Couldn't get chg_psy\n", __func__);
		} else {
			ret = power_supply_set_property(chg_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &propval);
			if (ret < 0) {
				pr_err("set power_role flag fail(%d)\n", ret);
			}
		}
		break;
	case TCP_NOTIFY_DR_SWAP:
		...
		if (noti->swap_state.new_role == PD_ROLE_UFP) {
			...
			if (chg_psy == NULL || IS_ERR(chg_psy)) {
				...
				ret = power_supply_set_property(chg_psy, POWER_SUPPLY_PROP_VOLTAGE_MAX, &propval);
				if (ret < 0) {
					pr_err("set data_role flag fail(%d)\n", ret);
				}
			}
		}
		break;
...
extern int aw_port_type_set(int type);
#endif
				    ...
				    enum typec_port_type type)
{
	...
	case TYPEC_PORT_SRC:
#if defined (CONFIG_N28_CHARGER_PRIVATE)
		if (rpmd && (rpmd->pd_conntected == PD_CONNECT_TYPEC_ONLY_SNK_DFT)) {
			dev_info(rpmd->dev, "%s: it's sink only, do not swap to src\n",
				__func__);
			chg_psy = power_supply_get_by_name("primary_chg");
			if (chg_psy == NULL || IS_ERR(chg_psy)) {
				dev_info(rpmd->dev,"%s Couldn't get chg_psy\n", __func__);
				...
				propval.intval = 0;
				ret = power_supply_set_property(chg_psy, POWER_SUPPLY_PROP_INPUT_VOLTAGE_LIMIT, &propval);
				if (ret < 0) {
					pr_err("set pr_role flag fail(%d)\n", ret);
				}
			}
			return -EOPNOTSUPP;
		}
