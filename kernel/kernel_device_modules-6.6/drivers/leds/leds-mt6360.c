...
//+bugS96901AA5-742,daizheng.wt,MODIFY,2024/09/04,chown enable_flashlight for factory
...
static const struct mt6360_led_platform_data def_platform_data = {
	.rgbon_sync = 0,
	.fled1_ultraistrb = 0,// bit7 ,12.5ma step ,50~1500ma
	.fled2_ultraistrb = 1,
};
...
{
	struct led_classdev_flash *flcdev1;
	struct led_classdev_flash *flcdev2;
	struct led_classdev *lcdev1;
	struct led_classdev *lcdev2;

	flcdev1 = mt6360_flash_class[0]; //MT6360_CHANNEL_CH1
	flcdev2 = mt6360_flash_class[1]; //MT6360_CHANNEL_CH2
	lcdev1 = &flcdev1->led_cdev;
	lcdev2 = &flcdev2->led_cdev;

	if (!strncmp("1",buf,1))
	{
		pr_info("enable_torch: torch on");
		mt6360_fled_brightness_set(lcdev1, 0);
		mt6360_fled_brightness_set(lcdev2, 0);
		msleep(100);
		mt6360_fled_brightness_set(lcdev1, 2);
		mt6360_fled_brightness_set(lcdev2, 2);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	pr_info("flashlight_show_fswitch\n");
	return 1;
}

static ssize_t flash_proc_write(struct file *file, const char *buffer,
					size_t count, loff_t *data)
{
	...
	if (!strncmp("1",buf,1))
	{
		...
		mt6360_fled_brightness_set(lcdev2, 0);
		msleep(100);
		mt6360_fled_brightness_set(lcdev1, 1);
		mt6360_fled_brightness_set(lcdev2, 1);
	}
	else if (!strncmp("0",buf,1))
	{
		pr_info("enable_torch: torch off");
...
static const struct proc_ops fled_fops = {
 .proc_write = flash_proc_write,
};

static int flash_proc_init(struct platform_device *pdev)
{
	int ret = 0;
	struct proc_dir_entry *proc_entry;

	proc_entry = proc_create("driver/enable_flashlight", 0664, NULL, &fled_fops);
	if (proc_entry == NULL) {
		ret = -ENOMEM;
		dev_err(&pdev->dev, "[%s]: Error! Couldn't create qcom_flash proc entry\n", __func__);
 	} else {
		pr_info("Success to create device file(driver/enable_flashlight)\n");
	}
 	return ret;
}
#endif
...
//+bugS96901AA5-742,daizheng.wt,MODIFY,2024/09/04,chown enable_flashlight for factory
#if IS_ENABLED(CONFIG_MTK_FLASHLIGHT)
static DEVICE_ATTR(fswitch, 0664, cam_flash_switch_show,cam_flash_switch_store);

void cam_flash_proc_init(struct platform_device *pdev)
{
	if (flash_proc_init(pdev) < 0) {
		if (device_create_file(&pdev->dev, &dev_attr_fswitch)) {
			pr_info("Failed to create device file(fswitch)\n");
		} else {
			pr_info("Success to create device file(fswitch)\n");
		}
	}
}
#endif
